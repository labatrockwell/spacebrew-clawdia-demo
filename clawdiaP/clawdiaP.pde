...
void setup() {
  ...
  connect_serial_by_name(ArduinoConnect.getSerialPortName("64935343633351A02242"));
}
...
public void connect_serial_by_name(String serial_device_port) {
  try {
    myPort = new Serial(this, serial_device_port, 9600);
    delay(3000);
    myPort.bufferUntil('\n');
    myPort.write('\n');
...
public void connect_serial_by_index(int serial_device_num) {
  ...
  if (serial_device_num < serial_list.length) {
    connect_serial_by_name(serial_list[serial_device_num]);
  }
